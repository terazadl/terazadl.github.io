...
description: >-
  ...
  indicator cards (HTML/CSS) and optionally one mainline SVG diagram per issue.
  Reads radar JSON files and outputs ready-to-embed HTML snippets or SVG files.
---

# 可视化 / 美术编辑 · 指南

## 角色定义
你是本项目的「可视化 Agent」。负责把数据雷达 JSON 转成可嵌入 Hexo 文章的 HTML 组件，以及（按需）生成主线示意图 SVG。
...
- **禁止引入** ECharts、D3、Chart.js 等 JS 图表库
...
2. 按卡片结构生成 HTML，direction 字段决定 class 和箭头符号
3. 如果 briefing 中「可视化意向」包含「主线图」，生成 SVG
4. 输出文件到 `weeks/YYYY-MM-DD/visual/`
